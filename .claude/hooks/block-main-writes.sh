...
#
# Every change gets to the default branch through a reviewed PR (see
# CLAUDE.md). This hook is a guard rail, not a lock: for a genuine exception,
# run the git command yourself in a terminal. Installed by the dev-workflow
# plugin. Keep it bash 3.2 compatible (macOS /bin/bash).
set -euo pipefail

input="$(cat)"

# Fast exit for commands that cannot be a git commit or push.
case "$input" in
  *git*commit* | *git*push*) ;;
  *) exit 0 ;;
esac
...
# git [global options] commit|push
...
Code does not commit to \`$default\` or push to it.
  ...
  Start a branch:  git worktree add -b <type>/<name> ../<repo>-<name> origin/$default
